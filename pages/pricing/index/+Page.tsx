...
export default function Page() {
    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4">
            {/* Header Section */}
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-1">
                ...
                <button
                    ...
                    className="group relative flex items-center gap-2 bg-slate-900 dark:bg-emerald-600 text-white px-6 py-4 rounded-3xl font-bold shadow-xl hover:shadow-emerald-500/20 hover:scale-[1.02] transition-all overflow-hidden active:scale-95"
                >
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {['MISSION', 'VOYAGE', 'COMMANDE'].map((tpl) => (
                    <div key={tpl} className="bg-white dark:bg-slate-900/50 p-4 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 group hover:border-emerald-500/30 transition-colors">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${activeByTemplate[tpl] ? 'bg-emerald-50 dark:bg-emerald-500/10' : 'bg-slate-50 dark:bg-slate-800'}`}>
                            {tpl === 'MISSION' && <Activity className={activeByTemplate[tpl] ? 'text-emerald-500' : 'text-slate-400'} size={24} />}
                            {tpl === 'VOYAGE' && <Globe className={activeByTemplate[tpl] ? 'text-emerald-500' : 'text-slate-400'} size={24} />}
                            {tpl === 'COMMANDE' && <Layers className={activeByTemplate[tpl] ? 'text-emerald-500' : 'text-slate-400'} size={24} />}
                        </div>
                        <div className="flex-1">
                            <p className="text-[10px] uppercase font-black text-slate-400 tracking-tighter">{tpl}</p>
                            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">
                                {activeByTemplate[tpl] || <span className="text-slate-400 font-medium italic">Aucun actif</span>}
                            </h4>
                        </div>
                    </div>
                ))}
...
function PricingEditor({ isOpen, onClose, filter, onSave }: { isOpen: boolean, onClose: () => void, filter: PricingFilter | null, onSave: (data: Partial<PricingFilter>) => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] overflow-hidden">
            ...
            {/* Slide-over */}
            <div className="absolute inset-y-0 right-0 max-w-2xl w-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 ease-out">
                <header className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20 backdrop-blur-md sticky top-0 z-10">
                    <div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-hide pb-32">
                    ...
                    <section className="space-y-6">
                        ...
                        <div className="space-y-8 bg-slate-50 dark:bg-slate-800/30 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-inner">
                            {/* Proximity */}
                ...
                <footer className="p-8 border-t border-slate-100 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl flex gap-4 sticky bottom-0 z-10">
                    <button
                        onClick={onClose}
...
function PricingCard({ filter, handleEdit, handleToggleActive, handleDelete }: { filter: PricingFilter, handleEdit: (filter: PricingFilter) => void, handleToggleActive: (filterId: string) => void, handleDelete: (id: string) => void }) {
    ...
    return (
        <div className={`group relative bg-white dark:bg-slate-900 rounded-[3rem] border-2 transition-all duration-500 overflow-hidden ${filter.isActive
            ...
            {/* Status Ribon */}
            {filter.isActive && (
                <div className="absolute top-8 right-8 flex items-center gap-2 bg-emerald-500 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.1em] shadow-lg shadow-emerald-500/30">
            ...
            <div className="p-10">
                ...
                <div className="grid grid-cols-2 gap-6 mb-10">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 flex flex-col items-center gap-2 group/metric">
                ...
                <div className="space-y-6">
                    ...
                    {expanded && (
                        <div className="space-y-8 pt-6 border-t border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-4 duration-500">
                            {/* Multipliers row */}
                            <div className="grid grid-cols-3 gap-3">
                                <MultiplierMiniBadge label="Urgent" value={filter.urgentMultiplier || 1} color="orange" />
                                <MultiplierMiniBadge label="Nuit" value={filter.nightMultiplier || 1} color="purple" />
                                <MultiplierMiniBadge label="Fragile" value={filter.fragileMultiplier || 1} color="blue" />
                            </div>

                            {/* Advanced Rules Grid */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {/* Discount Rule */}
                                <div className="p-5 rounded-3xl bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-100 dark:border-emerald-500/20 space-y-2">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Remise Proximité</span>
                                        <span className="text-lg font-black text-emerald-600">-{filter.proximityDiscountPercent}%</span>
                                    </div>
                                    <p className="text-[10px] font-bold text-emerald-600/60 leading-tight">Applicable si le prochain stop est à moins de {filter.proximityThresholdKm}km.</p>
                                </div>

                                {/* Heavy Load Rule */}
                                <div className="p-5 rounded-3xl bg-rose-50 dark:bg-rose-500/5 border border-rose-100 dark:border-rose-500/20 space-y-2">
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Surcharge Lourde</span>
                                        <span className="text-lg font-black text-rose-600">+{filter.heavyLoadSurchargePercent}%</span>
                                    </div>
                                    <p className="text-[10px] font-bold text-rose-600/60 leading-tight">Surcharge appliquée pour toute charge supérieure à {filter.heavyLoadSurchargeThresholdKg}kg.</p>
                                </div>
                            </div>

                            {/* Distance Limits */}
                            <div className="p-6 rounded-[2rem] bg-slate-900 border border-slate-800 text-white shadow-xl shadow-slate-900/20">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Périmètre d'Application</h4>
                                    <Info size={14} className="text-slate-600" />
                                </div>
                                <div className="grid grid-cols-2 gap-8">
                                    <div className="space-y-1">
                                        <span className="text-2xl font-black block">{filter.minDistance}km</span>
                                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Distance Minimale</span>
                                    </div>
                                    <div className="space-y-1">
                                        <span className="text-2xl font-black block">{filter.maxDistance ? `${filter.maxDistance}km` : 'Illimité'}</span>
                                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Rayon Maximal</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <footer className="mt-10 flex gap-4">
                    <button
                        ...
                        className="flex-1 py-5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-[1.8rem] font-black text-xs uppercase tracking-widest transition-all group/expand"
                    >
                        {expanded ? 'Fermer les détails' : 'Analyser les charges'}
                        <ChevronRight size={14} className={`inline-block ml-2 transition-transform ${expanded ? '-rotate-90' : 'rotate-90 group-hover/expand:translate-y-0.5'}`} />
                    </button>
                    {!filter.isActive ? (
                        <button
