...
const StopDetailPanel: React.FC<StopDetailPanelProps> = ({
    ...
    const [view, setView] = useState<'stop' | 'product' | 'validation-edit'>('stop');
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide p-6 space-y-8">
                    ...
                    <section>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-[12px] uppercase tracking-widest text-gray-400 font-bold">General Information</h3>
                        </div>
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        <EditableField
                            ...
                            value={stop.address?.street || stop.address || ''}
                            placeholder="123 Main St"
                            onChange={(val) => handleFieldChange('address.street', val)}
                        />
                        <div className="grid grid-cols-2 gap-4">
                            <EditableField
                                label="City"
                        ...
                        <button
                            onClick={() => {
                                const newAction = {
                                    ...
                                    weight: 0,
                                    unitaryPrice: 0,
                                    type: stop.type === 'Pick-Up' ? 'pickup' : 'delivery',
                                    requirements: ['sec'],
                                    status: 'Pending',
                                    secure: 'none'
                                };
                    ...
                    <div className="space-y-3">
                        {(stop.actions || []).map((action: any, idx: number) => (
                            <div
                                ...
                                className=" bg-white rounded-2xl p-2 shadow-sm relative group/action border border-transparent hover:border-blue-100/50 cursor-pointer transition-all"
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex flex-col flex-1 pl-1">
                                        <span className="text-xs font-bold text-gray-900 group-hover/action:text-blue-600 transition-colors line-clamp-1 break-all">
                                            {action.productName || action.product?.name || 'New Product'}
                                        </span>
                                        <span className="text-[10px] text-gray-400 mt-0.5">
                                            Qty: {action.quantity || 1} • {action.type}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <button
                                            ...
                                            className="p-1.5 text-gray-300 hover:text-rose-500 rounded-lg hover:bg-rose-50 transition-all opacity-0 group-hover/action:opacity-100"
                                            title="Remove Product"
                                        >
                                            <Trash2 size={14} />
                                        </button>
                                        <ChevronRight size={14} className="text-gray-300 group-hover/action:text-blue-400 group-hover/action:translate-x-0.5 transition-all" />
                                    </div>
                                </div>
                            </div>
                        ))}
